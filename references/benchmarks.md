# Industry Benchmarks — CEO Reference

Use this file to contextualize any metric the CEO shares. When presenting numbers, always show the benchmark alongside the actual.

Format: **"Your number: X — Benchmark for [stage/type]: Y — Status: ✅ / ⚠️ / 🔴"**

**Sources:** Bessemer Venture Partners State of the Cloud, OpenView SaaS Benchmarks, KeyBanc Capital Markets SaaS Survey, Paddle/ProfitWell subscription benchmarks, Battery Ventures SaaS reports, ChartMogul subscription data. Regional data: NASSCOM, e-Conomy SEA (Google/Temasek/Bain), Wamda MENA Tech reports, Tech Nation UK.

---

## How to use benchmarks

1. Check the CEO's company stage from `company-profile.md`
2. Check the company type (SaaS, services, ecommerce, marketplace)
3. Find the relevant benchmark below
4. Present it inline when showing metrics — never make the CEO ask "is this good?"

---

## SaaS Benchmarks

### Revenue Growth
| Stage | Good | Watch | Danger |
|-------|------|-------|--------|
| Early ($0–$500K ARR) | >15% MoM | 5–15% MoM | <5% MoM |
| Growth ($500K–$5M ARR) | >10% MoM | 3–10% MoM | <3% MoM |
| Scale ($5M+ ARR) | >5% MoM | 1–5% MoM | <1% MoM |

### Gross Margin
| Type | Good | Watch | Danger |
|------|------|-------|--------|
| SaaS / Software | >70% | 50–70% | <50% |
| SaaS + Services | >50% | 35–50% | <35% |
| Marketplace | >60% | 40–60% | <40% |

### Churn Rate (Monthly)
| Stage | Good | Watch | Danger |
|-------|------|-------|--------|
| Early | <3% | 3–5% | >5% |
| Growth | <2% | 2–4% | >4% |
| Scale | <1% | 1–2% | >2% |

### Net Revenue Retention (NRR)
| Level | What it means |
|-------|--------------|
| >120% | Excellent — customers are expanding faster than churning |
| 100–120% | Good — growing from existing base |
| 90–100% | Watch — barely replacing churn |
| <90% | Danger — shrinking from existing customers |

### CAC Payback Period
| Stage | Good | Watch | Danger |
|-------|------|-------|--------|
| Early | <18 months | 18–24 months | >24 months |
| Growth | <12 months | 12–18 months | >18 months |
| Scale | <6 months | 6–12 months | >12 months |

### LTV:CAC Ratio
| Ratio | Status |
|-------|--------|
| >5:1 | Excellent — underspending on growth |
| 3:1–5:1 | Good — healthy unit economics |
| 1:1–3:1 | Watch — marginal or unsustainable |
| <1:1 | Danger — losing money on every customer |

### Sales Quota Attainment
| Level | Status |
|-------|--------|
| >80% team hitting quota | Excellent |
| 60–80% hitting quota | Good |
| 40–60% hitting quota | Watch |
| <40% hitting quota | Danger |

### Magic Number (Sales Efficiency)
| Score | What it means |
|-------|--------------|
| >1.0 | Excellent — very efficient growth |
| 0.75–1.0 | Good — spend more on sales/marketing |
| 0.5–0.75 | Watch — efficiency improving needed |
| <0.5 | Danger — growth is too expensive |

### Rule of 40
Revenue growth rate % + Profit margin % should be ≥ 40

| Score | Status |
|-------|--------|
| >40 | Healthy business |
| 20–40 | Acceptable, room to improve |
| <20 | Under pressure |

---

## Services / Agency Benchmarks

### Utilization Rate
| Level | Status |
|-------|--------|
| 75–85% | Optimal — above this burns out team, below wastes capacity |
| 60–75% | Watch — capacity being wasted |
| >85% | Watch — team at risk of burnout, can't take new work |

### Gross Margin
| Level | Status |
|-------|--------|
| >40% | Good |
| 25–40% | Acceptable |
| <25% | Danger — pricing or delivery cost problem |

### Revenue per Employee
| Level | Status |
|-------|--------|
| >$150K/year | Good |
| $100–150K/year | Acceptable |
| <$100K/year | Watch — team may be underpriced or overstaffed |

---

## Ecommerce Benchmarks

### Customer Acquisition Cost vs AOV
LTV should be at least 3x CAC. AOV (Average Order Value) × purchase frequency = LTV

### Conversion Rate (website to purchase)
| Channel | Good | Watch | Danger |
|---------|------|-------|--------|
| Direct traffic | >3% | 1–3% | <1% |
| Paid traffic | >2% | 0.5–2% | <0.5% |
| Email | >5% | 2–5% | <2% |

### Cart Abandonment Rate
| Level | Status |
|-------|--------|
| <60% | Good |
| 60–75% | Acceptable |
| >75% | Danger — checkout friction |

### Return Rate
| Level | Status |
|-------|--------|
| <15% | Good |
| 15–25% | Watch |
| >25% | Danger — product-market or quality issue |

---

## Universal Benchmarks (all company types)

### Runway
| Months | Status |
|--------|--------|
| >18 months | Safe |
| 12–18 months | Start planning next raise or path to profitability |
| 6–12 months | Urgent — fundraise or cut now |
| <6 months | Critical — immediate action required |

### Employee Attrition (Annual)
| Level | Status |
|-------|--------|
| <10% | Low — healthy retention |
| 10–20% | Normal — monitor by department |
| >20% | High — investigate root cause |
| >30% | Crisis — systemic problem |

### NPS Score
| Score | Status |
|-------|--------|
| >70 | World class |
| 50–70 | Excellent |
| 30–50 | Good |
| 0–30 | Needs improvement |
| <0 | Danger — more detractors than promoters |

### Support Response Time (B2B SaaS)
| Tier | Good | Watch | Danger |
|------|------|-------|--------|
| Critical | <1 hr | 1–4 hrs | >4 hrs |
| High | <4 hrs | 4–8 hrs | >8 hrs |
| Normal | <24 hrs | 24–48 hrs | >48 hrs |

---

## Regional Benchmarks

Load these when `company-profile.md` shows a non-US primary operating region. Use regional benchmarks alongside the global SaaS/services benchmarks above.

---

### South Asia (Bangladesh, India, Pakistan, Sri Lanka)

**SaaS Revenue Growth (INR/BDT-denominated businesses)**
| Stage | Good | Watch | Danger |
|-------|------|-------|--------|
| Early (<$200K ARR equiv.) | >20% MoM | 8–20% MoM | <8% MoM |
| Growth ($200K–$2M ARR equiv.) | >12% MoM | 4–12% MoM | <4% MoM |
| Scale ($2M+ ARR equiv.) | >6% MoM | 2–6% MoM | <2% MoM |

**Gross Margin (South Asia SaaS)**
| Type | Good | Watch | Danger |
|------|------|-------|--------|
| SaaS / Software | >65% | 45–65% | <45% |
| IT Services / Agency | >30% | 18–30% | <18% |
| BPO / Outsourcing | >20% | 12–20% | <12% |

**Salary benchmarks (USD equivalent, approximate)**
| Role | Bangladesh | India | Pakistan |
|------|-----------|-------|---------|
| Software Engineer (mid) | $8–18K/yr | $12–30K/yr | $8–18K/yr |
| Sales (mid) | $5–12K/yr | $8–20K/yr | $5–12K/yr |
| Marketing (mid) | $5–10K/yr | $8–18K/yr | $5–10K/yr |
| VP / Head of (senior) | $15–35K/yr | $25–60K/yr | $15–30K/yr |

**CAC Payback (South Asia B2B SaaS)**
- Good: <24 months
- Watch: 24–36 months
- Danger: >36 months

*Note: Longer payback acceptable at early stage — South Asia enterprise sales cycles are longer.*

**Revenue per employee (IT services)**
| Level | Status |
|-------|--------|
| >$50K/yr | Good for South Asia market |
| $30–50K/yr | Acceptable |
| <$30K/yr | Watch — pricing or staffing issue |

---

### Southeast Asia (Singapore, Indonesia, Vietnam, Philippines, Thailand, Malaysia)

**SaaS Growth (SGD/USD regional businesses)**
| Stage | Good | Watch | Danger |
|-------|------|-------|--------|
| Early | >18% MoM | 6–18% MoM | <6% MoM |
| Growth | >10% MoM | 3–10% MoM | <3% MoM |
| Scale | >5% MoM | 1–5% MoM | <1% MoM |

**Salary benchmarks (USD equivalent)**
| Role | Singapore | Vietnam/Philippines | Indonesia |
|------|-----------|-------------------|-----------|
| Software Engineer (mid) | $60–100K/yr | $15–35K/yr | $15–30K/yr |
| Sales (mid) | $50–80K/yr | $12–25K/yr | $12–22K/yr |
| VP / Head of | $100–160K/yr | $25–55K/yr | $22–50K/yr |

**Churn (Southeast Asia SMB SaaS)**
- Good: <4% monthly (SMB churn is structurally higher in SEA)
- Watch: 4–7% monthly
- Danger: >7% monthly

---

### Middle East (UAE, Saudi Arabia, Qatar, Egypt)

**Enterprise deal benchmarks (B2B SaaS/tech)**
| Metric | UAE/KSA Enterprise | Egypt SMB |
|--------|-------------------|----------|
| Average sales cycle | 3–9 months | 1–3 months |
| Average deal size | $20K–$200K/yr | $3K–$20K/yr |
| Win rate (typical) | 15–30% | 20–40% |

**Salary benchmarks (USD)**
| Role | UAE / Saudi | Egypt |
|------|------------|-------|
| Software Engineer (mid) | $40–80K/yr | $10–20K/yr |
| Sales (mid) | $35–70K/yr | $8–15K/yr |
| VP / Head of | $80–150K/yr | $20–40K/yr |

*UAE figures include housing allowance and benefits typical in Gulf packages.*

**Gross margin (Middle East tech services)**
| Type | Good | Watch |
|------|------|-------|
| SaaS | >65% | 45–65% |
| Tech consulting / SI | >35% | 20–35% |
| Managed services | >25% | 15–25% |

---

### UK & Europe

**SaaS Growth (GBP/EUR businesses)**
| Stage | Good | Watch | Danger |
|-------|------|-------|--------|
| Early | >12% MoM | 4–12% MoM | <4% MoM |
| Growth | >8% MoM | 2–8% MoM | <2% MoM |
| Scale | >4% MoM | 1–4% MoM | <1% MoM |

**Salary benchmarks (USD equivalent)**
| Role | UK (London) | Germany | Eastern Europe |
|------|------------|---------|----------------|
| Software Engineer (mid) | $70–120K/yr | $65–110K/yr | $25–55K/yr |
| Sales (mid) | $55–90K/yr | $55–85K/yr | $20–40K/yr |
| VP / Head of | $120–200K/yr | $110–180K/yr | $45–90K/yr |

**Churn (European enterprise SaaS)**
- Good: <1% monthly (EU enterprise expects longer contracts)
- Watch: 1–2% monthly
- Danger: >2% monthly

---

## How to present benchmarks in responses

Always use this format when showing a metric:

> **MRR: $85,000** ↑ 12% MoM
> Benchmark (Growth stage SaaS): 10% MoM is good. You're above — strong signal.

> **Churn: 4.2%** ↑ from 3.8% last month
> Benchmark (Growth stage SaaS): Good is <2%, Watch is 2–4%. You're in the danger zone — this needs immediate attention.

Never just show the number. Always show the benchmark and the interpretation.
